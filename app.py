...
# ── Load lite PSA GeoJSON (once at startup, 0.94MB) ───────────────────────────
...
# ── Build ee.FeatureCollection from lite features ────────────────────────────
        ...
        for gacc in GACC_CODES:
            ...
            try:
                ...
                stats_fc = stack.reduceRegions(
                    collection=psa_fc, reducer=ee.Reducer.mean(), scale=90, tileScale=16,
                )
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app:app", host="0.0.0.0", port=int(os.getenv("PORT","8000")))    "EE_COLLECTION_16D_PROV",
    "projects/rap-data-365417/assets/npp-partitioned-16day-v3-provisional",
)
EE_COLLECTION_16D_ARCH = os.getenv(
    "EE_COLLECTION_16D_ARCH",
